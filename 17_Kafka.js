...
Apache Kafka is a distributed, fault-tolerant event streaming platform designed for high-throughput, low-latency ingestion and processing of real-time data streams. It solves problems of decoupling producers and consumers, handling massive message volumes, and reliably storing and distributing events for use cases like logging, metrics, event-driven architectures, and data pipelines.
...
- Producer:
  ...
  - Can control partitioning, acks, retries, batching, and compression.
...
- Consumer Group:
  ...
  - Each partition in a topic is consumed by only one consumer instance within the group at a time.

- Parallelism and scalability:
  - Adding more consumers in the same group increases parallelism up to the number of partitions.
  - Rebalance:
    - When consumers join/leave, Kafka redistributes partitions across consumers.
...
- Retention policy:
  - Defines how long or how much data Kafka keeps per topic.
  - Kafka is not a traditional queue; it retains messages based on time/size, not consumption.
...
- ISR (In-Sync Replicas):
  - Set of replicas that are fully caught up with the leader within a certain lag threshold.
  - Includes the leader and followers that are synchronized.
...
- Offset:
  - A monotonically increasing integer identifying a message’s position within a partition log.
  - Acts as a pointer for consumers to track read progress.
...
- Log compaction:
  ...
  - Useful for changelog topics, state reconstruction, and durable key-value snapshots.
...
- Partition key:
  ...
  - If key is null:
    - Default partitioner may round-robin among partitions.
  - If key is provided:
    - Partition is chosen by hashing the key (e.g., hash(key) % numPartitions).
...
- Effects:
  ...
  - Uneven key distribution can cause partition skew (hot partitions), affecting performance.
...
- Transactional messaging:
  ...
  - Uses transactional.id and transaction APIs:
    - beginTransaction, send, sendOffsetsToTransaction, commitTransaction/abortTransaction.
...
- Kafka:
  ...
  - Excellent for event sourcing, stream processing, and data pipelines.
...
- Approach:
  - Use a schema-aware serialization format:
    - Avro, Protobuf, or JSON with schema.
  - Use a Schema Registry:
    - Stores schemas centrally and assigns schema IDs.
    - Producers write a schema ID plus serialized data; consumers fetch schema by ID.
...
- Schema evolution:
  ...
  - Consumers/producers:
    - Can be upgraded independently as long as compatibility rules are honored.
...
- Kafka Connect:
  - A framework for scalable, fault-tolerant integration between Kafka and external systems (DBs, file systems, SaaS).
...
- Kafka Connect:
  - Framework to move data between Kafka and other systems with minimal custom code.
  - Provides configuration-based deployment of connectors.
...
- Monitoring metrics:
  ...
  - Producer:
    - Record send rate, batch sizes, retries, errors.
  - Consumer:
...
- Tools:
  ...
  - Confluent Control Center, LinkedIn Burrow, Cruise Control.
  - Logs and topic-level metrics.

- Tuning:
  - Producer configs: batch.size, linger.ms, compression.type, acks.
  - Broker configs: num.partitions, replication factor, log.segment.bytes.
  - Consumer configs: max.poll.records, fetch.min.bytes, session.timeout.ms.

21. What is backpressure in Kafka consumers and how do you handle it?

- Backpressure:
  - Condition where consumers cannot keep up with the rate of incoming messages.
  - Leads to growing consumer lag and potential resource exhaustion in downstream systems.

- Handling strategies:
  - Scale out:
    - Add more consumers in the group (up to number of partitions).
  - Tune consumer:
    - Increase max.poll.records, optimize processing logic, use async I/O.
  - Apply flow control:
    - Use rate-limiting, buffering, or batching downstream.
  - Use backpressure-aware frameworks:
    - Reactive streams, Kafka Streams with appropriate configuration.
  - If necessary, reduce producer rate or apply throttling.
...
- Failover:
  - For each affected partition:
    - A new leader is elected from ISR replicas (if available).
  - Producers and consumers:
    - Receive updated metadata and redirect requests to new leaders.
  - If no ISR available:
...
- Encryption:
  ...
  - Protect data in transit and mitigate eavesdropping.
