...
/*
TOP 25 MOST ASKED AND MOST IMPORTANT KUBERNETES INTERVIEW QUESTIONS - ANSWERED

1. What is Kubernetes and what problems does it solve in container orchestration?

Kubernetes is an open-source container orchestration platform that automates the deployment, 
scaling, and management of containerized applications across clusters of machines. It solves 
critical problems in container orchestration:
...
Before Kubernetes, managing containers at scale required complex custom scripts and manual 
intervention. Kubernetes provides a declarative approach where you define the desired state 
and it maintains that state automatically.
...
A Pod is the smallest and simplest unit in Kubernetes that represents a single instance 
of a running process in your cluster. Key characteristics:
...
- Abstraction Layer: Kubernetes manages Pods, not individual containers, providing a 
  higher-level abstraction for deployment
- Co-location: Enables tightly coupled containers to run together (main app + helper containers)
- Resource Sharing: Containers in a Pod can communicate via localhost and share volumes
- Atomic Unit: The Pod is the atomic unit of scheduling and scaling
- Simplified Management: Managing Pods instead of individual containers reduces complexity
- Multi-container Patterns: Supports sidecar, ambassador, and adapter patterns

Example use case: A web server container and a log collector container running together 
in the same Pod, sharing the same log volume.
...
When you create a Deployment, it automatically creates a ReplicaSet. During rolling updates,
...
- Directly use ReplicaSets only in very specific scenarios where you need fine-grained 
  control without update management


5. Explain the purpose and differences between Services (ClusterIP, NodePort, LoadBalancer, ExternalName).

Services provide stable networking endpoints for accessing Pods. They abstract Pod IPs 
which change frequently due to scaling and rescheduling.
...
Namespaces provide virtual clusters within a physical Kubernetes cluster, enabling 
resource isolation and organization.

KEY FEATURES:

- Logical partitioning of cluster resources
- Scope for names (names must be unique within a namespace, not across namespaces)
...
- Access control: Apply different RBAC policies to different namespaces
- Resource quotas: Limit CPU/memory consumption per team/project
- Large organizations: When managing many users and applications
...
IMPORTANT: Base64 encoding in Secrets is NOT encryption. For production security, 
enable encryption at rest or use external secret management solutions like HashiCorp 
Vault, AWS Secrets Manager, or Azure Key Vault.


9. What are PersistentVolumes (PV) and PersistentVolumeClaims (PVC)?

Kubernetes provides an abstraction layer for storage management through PVs and PVCs, 
separating storage provisioning from consumption.
...
- Check specific revision: kubectl rollout history deployment/myapp --revision=2
- Rollback to previous: kubectl rollout undo deployment/myapp
- Rollback to specific revision: kubectl rollout undo deployment/myapp --to-revision=3
...
- Restart deployment: kubectl rollout restart deployment/myapp
...
StatefulSet manages stateful applications that require stable network identities, 
persistent storage, and ordered deployment/scaling.
...
DaemonSet ensures that all (or specific) nodes run a copy of a Pod. As nodes join 
the cluster, Pods are added automatically. When nodes are removed, Pods are garbage collected.
...
enabling Pods to find and communicate with Services automatically.
...
Ingress provides HTTP/HTTPS routing to Services based on rules, enabling external 
access with features like SSL termination, name-based virtual hosting, and path-based routing.
...
Probes are health checks that kubelet performs on containers to ensure application 
availability and proper lifecycle management.
...
Important: If liveness probe fails repeatedly, Pod enters CrashLoopBackOff state.
...
Runs throughout Pod lifecycle, not just at startup.
...
Once startup probe succeeds, it doesn't run again. Liveness probe takes over.

Prevents scenario: App takes 60 seconds to start, but liveness probe fails after 10 
seconds and kills container prematurely.
...
Kubernetes manages container resources through requests and limits to ensure efficient 
cluster utilization and prevent resource starvation.
...
Helm is a package manager for Kubernetes that simplifies deployment, versioning, and 
management of applications through reusable templates called charts.
...
Kubernetes implements a flat networking model with specific requirements that enable 
...
Kubernetes provides three autoscaling mechanisms working at different layers to 
...
/*
COMPLETING QUESTIONS 22-25


22. What is RBAC (Role-Based Access Control) in Kubernetes? (CONTINUED)

EXAMPLE SCENARIOS (continued):

1. Developer Role (namespace-scoped):
   - Can create/update Deployments and Services
   - Can view Pods and logs
   - Cannot delete or access cluster-level resources
...
2. Read-Only User (cluster-wide):
   - Can view resources across all namespaces
...
3. ServiceAccount for Application:
   - Pod needs to read ConfigMaps and Secrets
   - Specific to application namespace
...
Definition:
- Increases/decreases number of identical Pod instances
- Distributes load across multiple Pods
- Pods run on same or different nodes
- Does not change resources per Pod
...
Definition:
- Changes resource requests/limits per Pod
- Increases power of individual Pods
- Same number of Pods, more resources each
- Single-instance capability improvement
...
- Primary: Horizontal scaling (HPA)
- Handle variable traffic
- Multiple replicas for availability
- Occasional: Vertical (VPA for optimization)

Scenario 2: PostgreSQL Database
- Primary: Vertical scaling (VPA)
- Single primary instance (write)
- Read replicas can scale horizontally
- Limited by single-writer architecture

Scenario 3: Microservices
- Horizontal scaling per service
- Each service independently scaled
- VPA for long-term optimization
...
What to look for:
- Events section (bottom): Shows timeline of issues
...
STEP 6: EXEC INTO POD (if running)
...
Check if resource exhaustion causing issues
...
Solutions:
  ...
  # Check readiness probe in describe
  kubectl describe pod <pod-name>
  ...
  readinessProbe:
    initialDelaySeconds: 30  # Allow more startup time
    periodSeconds: 10
    failureThreshold: 3
...
A method of packaging, deploying, and managing Kubernetes applications using custom 
resources and controllers that encode operational knowledge.

CONCEPT:

Operators extend Kubernetes to automate management of complex, stateful applications 
by codifying human operator knowledge into software.
...
Define custom resource spec (api/v1/myapp_types.go):
...
Implement controller logic (controllers/myapp_controller.go):
...
3. Cluster-Scoped:
   ...
   - Example: Node problem detector

4. Namespace-Scoped:
   - Manages resources within namespace
   - Multi-tenancy support
...
- Increased adoption for complex workloads
...
Operators are powerful extensions that bring intelligence to Kubernetes, automating 
complex application management and encoding operational expertise into self-managing 
software systems.
*/
